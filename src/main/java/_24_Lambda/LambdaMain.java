    ...
    public static void main(String[] args) {
        ...
        /*
            ...
            메서드를 객체처럼 다루고 싶다.
                ...
                - 리턴으로 메서드를 리턴하고 싶다.
            -> 메서드 하나만 가지는 객체를 만들자
            -> 익명클래스(한 번 쓰고 버리는 클래스)
         */

        // add는 익명클래스 객체 - 캐스팅되어있는 상태
        // Calculator 객체가 아님
        // Calculator를 implements받은 익명클래스 객체
        ...
        // multi는 Calculator implements받은 익명클래스2 객체
        ...
        // printResult는 매개변수로Calculator 타입 받음.
        printResult(10, 5, add); // 캐스팅 되어 있음
        printResult(10, 5, multi); // 캐스팅 되어 있음

        // 어차피 메서드 하나만 가지고 있음
        // 어차피 오버라이딩 해야함(인터페이스로 만들 익명클래스)
        // 메서드 이름, 매개변수타입 생략
        // 리턴을 한 줄에 작성할 수 있으면, {} 및 return 키워드 생략
        ...
        // 메서드 매개변수 자리에 메서드를 람다식으로 정의하면서 넘겨줌
        ...
        // StringChecker를 이용하여
        // 빈 문자열을 제외한 이름들만 trimNames에 추가해주세요.
        List<String> trimNames = new ArrayList<>();
        StringChecker notEmptyName = (s) -> s.length() > 0;
        for (String name : names) {
            if (notEmptyName.check(name)) {
                trimNames.add(name);
            }
        }
//        StringChecker isEmptyStr = s -> s.length() == 0; // s.isEmpty(); s.isBlank();
//        for (String name : names) {
